...
int SysTokenizer::open(const char*filename, SysTokenizer** outTokenizer) {
    *outTokenizer = NULL;
    ...
    if (fd < 0) {
        result = -errno;
        MESON_LOGE("Error opening file '%s', %s.", filename, strerror(errno));
    } else {
        struct stat stat;
        if (fstat(fd, &stat)) {
            result = -errno;
            MESON_LOGE("Error getting size of file '%s', %s.", filename, strerror(errno));
        } else {
            ...
            if (nrd < 0) {
                result = -errno;
                MESON_LOGE("Error reading file '%s', %s.", filename, strerror(errno));
                delete[] buffer;
                buffer = NULL;
            } else {
                length = size_t(nrd);
...
#if DEBUG_TOKENIZER
    MESON_LOGE("nextToken mCurrent:%s, delimiters:%s\n", mCurrent, delimiters);
#endif
...
#if DEBUG_TOKENIZER
    MESON_LOGE("nextToken parse end str:%s, num:%d \n", mStrs, mCurrent - tokenStart);
#endif
    return (char *)mStrs;
}
